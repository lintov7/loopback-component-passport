...
function UserIdentity(UserIdentity) {
  ...
  function profileToUser(provider, profile, options) {
    ...
    if (!options.profileMapping) {
      ...
      var profileEmail;
      if(profile.emails){
        profileEmail = profile.emails && profile.emails[0] && profile.emails[0].value;      
      }else if(profile.emailAddresses){
        profileEmail = profile.emailAddresses && profile.emailAddresses[0] && profile.emailAddresses[0].value;  
      }
      var generatedEmail =
  ...
  UserIdentity.login = function(
    ...
    userIdentityModel.findOne(
      ...
      function(err, identity) {
        ...
        userModel.findOrCreate({where: query}, userObj, { validate: false }, function(err, user) {
          if (err) {
            return cb(err);
          }
          var date = new Date();
